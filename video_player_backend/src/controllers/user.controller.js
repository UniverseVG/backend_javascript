...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  const isPasswordValid = await currentUser.isPasswordCorrect(oldPassword);
  if (!isPasswordValid) {
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, currentUser, "Account details updated successfully")
    );
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, currentUser, "Cover image updated successfully")
    );
});
